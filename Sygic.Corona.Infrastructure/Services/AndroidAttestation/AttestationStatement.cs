...
namespace Sygic.Corona.Infrastructure.Services.AndroidAttestation
{
    public sealed class AttestationStatement
    {
        ...
        public AttestationStatement(Dictionary<string, string> claims)
        {
            ...
            {
                bool.TryParse(claims["ctsProfileMatch"], out bool ctsProfileMatchLocal);
                CtsProfileMatch = ctsProfileMatchLocal;
            }

            if (claims.ContainsKey("basicIntegrity"))
            {
                bool.TryParse(claims["basicIntegrity"], out bool basicIntegrityLocal);
